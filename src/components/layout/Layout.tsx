...
const Layout: React.FC<LayoutProps> = ({children}) => {
    ...
    const getUserInitials = () => {
        const email = (auth.user?.profile?.upn as string) || (auth.user?.profile?.email as string) || (auth.user?.profile?.preferred_username as string) || 'User';
    ...
    const getDisplayName = (): string => {
        return (auth.user?.profile?.unique_name as string) || (auth.user?.profile?.email as string) || (auth.user?.profile?.preferred_username as string) || 'User';
    };
                                    ...
                                    <DropdownMenuLabel className="font-normal">
                                        <div className="flex flex-col space-y-1">
                                            ...
                                            <p className="text-xs leading-none text-muted-foreground">
                                                {(auth.user?.profile?.upn as string) || (auth.user?.profile?.email as string) || 'user@example.com'}
                                            </p>
                                        </div>
                                    </DropdownMenuLabel>
                                    <DropdownMenuSeparator/>
